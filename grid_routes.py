...
@grid_bp.route("/")
def grid_dashboard(): return render_template("grid_dashboard.html")

@grid_bp.route("/api/status")
def api_status(): return jsonify(get_engine().get_status())

@grid_bp.route("/api/start", methods=["POST"])
def api_start():
    try:
        if not get_engine().is_running(): get_engine().start()
        return jsonify({"ok": True, "active": get_engine().is_running()})
    except Exception as e: return jsonify({"ok": False, "error": str(e)}), 500

@grid_bp.route("/api/stop", methods=["POST"])
def api_stop():
    try: get_engine().stop(); return jsonify({"ok": True, "active": get_engine().is_running()})
    except Exception as e: return jsonify({"ok": False, "error": str(e)}), 500
...
def api_build():
    try:
        ...
        result = get_engine().build_grid(upper=data.get("upper"), lower=data.get("lower"), grid_count=data.get("grid_count"), investment=data.get("investment"))
...
def api_stats(): return jsonify(get_db().get_stats(get_engine().symbol))
