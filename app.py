    ...
    try:
        ...
        if not saved:
            ...
            try:
                _attrs = [
                    ...
                    "DCA_PULLBACK_WAIT_PCT",
                    "DCA_MAX_ENTRIES",
                    "DCA_CASCADE_ENABLED",
                    "DCA_CASCADE_LEVEL1_PCT",
                    "DCA_CASCADE_LEVEL2_PCT",
                    "DCA_SMART_REENTRY_ENABLED",
                    "DCA_SMART_REENTRY_PULLBACK_PCT",
                    "DCA_SMART_REENTRY_MIN_AI_CONF",
                    "DCA_COMPOUND_ENABLED",
                    "DCA_COMPOUND_RATIO",
                    "DCA_COMPOUND_MAX_TON",
                    "DCA_ADAPTIVE_TRIGGER_ENABLED",
                    "DCA_ADAPTIVE_FAST_MOVE_PCT",
                    "DCA_ADAPTIVE_FAST_DROP_PCT",
                    "LARGE_SELL_DCA_ENABLED",
                    "LARGE_SELL_DCA_TON",
                    "LARGE_SELL_MIN_TON",
                    "LARGE_SELL_COOLDOWN_SEC",
                    "PROFIT_PROTECT_ENABLED",
                    "PROFIT_PROTECT_TON",
                    "PROFIT_PROTECT_DROP_PCT",
                    "PROFIT_PROTECT_AI_SELL",
                    "FEE_PCT",
                    "DCA_REENTRY_COOLDOWN_SEC",
        ...
        if not (1 <= Config.DCA_DROP_TRIGGER_PCT <= 25):
            ...
            try:
                ...
                update_section(
                    "config", {"DCA_DROP_TRIGGER_PCT": Config.DCA_DROP_TRIGGER_PCT}
                )
            except Exception:
                pass
...
def api_config_set():
    ...
    if (v := num("dca_cascade_level2_pct", 5, 200)) is not None:
    ...
    if (v := num("dca_smart_reentry_pullback_pct", 1, 50)) is not None:
        Config.DCA_SMART_REENTRY_PULLBACK_PCT = v
    if (v := num("dca_smart_reentry_min_ai_conf", 30, 99)) is not None:
    ...
    if (v := num("dca_compound_ratio", 0.01, 0.9)) is not None:
    ...
    if (v := num("dca_adaptive_fast_move_pct", 1, 30)) is not None:
        Config.DCA_ADAPTIVE_FAST_MOVE_PCT = v
    if (v := num("dca_adaptive_fast_drop_pct", 1, 30)) is not None:
        Config.DCA_ADAPTIVE_FAST_DROP_PCT = v

    # Fast reentry / Scalp / DCA cooldown
    if (v := num("fast_reentry_pullback_pct", 0.5, 20)) is not None:
        Config.FAST_REENTRY_PULLBACK_PCT = v
    if (v := num("scalp_target_net_pct", 0.5, 50)) is not None:
        Config.SCALP_TARGET_NET_PCT = v
    if (v := num("dca_reentry_cooldown_sec", 0, 3600)) is not None:
        Config.DCA_REENTRY_COOLDOWN_SEC = int(v)
    # ATR-множители динамических целей
    ...
    if (v := num("large_sell_dca_ton", 10, 100000)) is not None:
        Config.LARGE_SELL_DCA_TON = v
    if (v := num("large_sell_min_ton", 50, 100000)) is not None:
        Config.LARGE_SELL_MIN_TON = v
    if (v := num("large_sell_cooldown_sec", 30, 86400)) is not None:
    ...
    if (v := num("dca_ai_sell_block_conf", 0, 100)) is not None:
        Config.DCA_AI_SELL_BLOCK_CONF = v
