...
## Текущая сессия — 05.08.2026

### Выполнено

- ✅ По просьбе пользователя закреплено постоянное правило: при любой разработке
  сначала создавать подробную задачу с целью, контекстом, ограничениями, этапами и
  критериями готовности; затем фиксировать решения и проверки по ходу работы; в
  конце записывать изменённые файлы, риски и точные следующие шаги.
- ✅ Правило добавлено в раздел «Пользовательские настройки» файла `replit.md`.
- ✅ Отдельно зафиксировано, что секреты, пароли, токены, мнемоники и персональные
  данные не должны попадать в задачи, память или документацию.

### Незавершённое

- ⏳ Задача импорта проекта остаётся открытой: пользователь ещё не выбрал, нужно ли
  устанавливать зависимости и запускать демо-превью, оставить проект только
  редактором, внести конкретные правки или деплоить на VPS.
- ⏳ Workflow `Start application` сейчас не запускается из-за отсутствующего
  `numpy`; устанавливать зависимости до выбора пользователя не следует.
