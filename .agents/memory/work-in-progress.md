...
| **Баг #1 фикс**: `_tick()` делал ранний return при active=False → AI-менеджер не запускался, сетка не могла сама активироваться после rebuild | ✅ |
| **Баг #2 фикс**: `_manage()` активировал только если `_paused_by_ai=True` → добавлен `has_levels` check | ✅ |
| **Баг #3 фикс**: `_get_ai_signal()` при SELL возвращал `(prob_down, conf)` вместо `(prob_up, conf)` → BUY=SELL=86% отображалось некорректно | ✅ |
| Все 3 фикса задеплоены на VPS (docker cp) и закоммичены | ✅ |
| Сетка активна, тикает, режим UNKNOWN, next SELL @ +5% | ✅ |
