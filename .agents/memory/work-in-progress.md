...
**Причина прыгания:** `_updatePortfolioTracker` (каждые 2 сек) писал `pt-total` как `"$625.92"`, а `renderWalletFull` (каждые 6 сек) — как `"431.9750 TON / $625.92"`. Теперь при активной вкладке «Кошелёк» только `renderWalletFull` управляет этими элементами.
