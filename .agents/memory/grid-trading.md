...
- Шаг: 5% (ATR-адаптивный: 2.81% ATR → шаг 5%, >5% ATR → шаг 8%)
...
- AI SELL ≥ 70% → пропустить BUY на этом тике (не ловить нож)
...
- ATR < 2% → step 3.5% (низкая волатильность, плотная сетка)
...
- Простое решение: проверять `len(trader.open_trades) == 0` перед Grid BUY (не открывать если позиция уже закрыта трейдером)
