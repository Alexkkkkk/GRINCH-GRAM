...
class DedustClient:
    ...
    def _build_params_cell(self, recipient, c1: int):
        ...
        ВАЖНО: поле salt НЕ должно быть случайным. Пул GRINCH/TON (CPMM-v2)
        парсит эту 256-битную область как часть полезной нагрузки и на ПОЧТИ
        ПОЛОВИНЕ случайных значений падает в compute с exit 9 (cell underflow) —
        TON возвращается, но газ сгорает. Это и была причина «только гас уходит»:
        на каждый своп генерировалась новая случайная соль, и ~55% покупок/продаж
        отскакивали. Реальные успешные кошельки используют ОДНУ стабильную соль и
        переиспользуют её. Поэтому берём детерминированную, проверенную эмуляцией
        «хорошую» соль = hash получателя (recipient всегда — кошелёк платформы):
        она стабильна и даёт exit 0 (проверено: при ней свопы доходят до конца,
        а слишком высокий min-out корректно даёт exit 30, а не exit 9).
        """
