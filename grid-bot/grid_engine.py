"""Core grid trading engine with GitHub error reporting."""
import json
import logging
import math
import os
...
class GridTradingEngine:
    ...
    def build_grid(self, upper: float = None, lower: float = None,
        ...
        with self._lock:
            ...
            if not price:
                err = "Cannot get price"
                if Config.REPORT_ERRORS:
                    self.reporter.create_issue("Build Grid Failed", f"Failed to get price for {self.symbol}")
                return {"ok": False, "error": err}
            grid_count = grid_count or Config.GRID_COUNT
            investment = investment or Config.TOTAL_INVESTMENT
            upper = upper or Config.UPPER_PRICE
            lower = lower or Config.LOWER_PRICE
            ...
            for p in all_prices:
                ...
                qty = 0
                if side == "buy" and usdt_per_buy >= Config.MIN_ORDER_USDT:
                    qty = usdt_per_buy / p
    ...
    def _loop(self):
        while not self._stop.is_set():
            try:
                ...
                if Config.REPORT_ERRORS:
                    try:
                        self.reporter.report_exception(e, context="GridEngine tick loop")
    ...
    def _tick(self):
        ...
        with self._lock:
            ...
            if self._should_recenter(price):
                ...
                self.build_grid()
                if Config.REPORT_ERRORS:
                    self.reporter.report_grid_rebuild(old_center, price, "Price drift exceeded threshold")
                return
            self._check_orders()
            self._update_stats()
            self._save_state()
    ...
    def _handle_fill(self, lvl: GridLevel, order: Dict):
        ...
        if lvl.side == "buy":
            ...
            if sell_lvl:
                sell_qty = executed_qty * (1 - Config.FEE_PCT / 100)
            ...
            if buy_lvl:
                ...
                if quote_qty >= Config.MIN_ORDER_USDT:
    ...
    def _update_stats(self):
        invested = Config.TOTAL_INVESTMENT
        if invested > 0:
