...
class AIEngine:
    ...
    def _analyze_locked(self, ohlcv: list) -> dict:
        ...
        if ai_signal == "BUY":
            # Momentum буст (независимый детектор скорости)
            mom_boost  = float(momentum.get("boost", 0.0))
            # Breakout буст (объём+BB+MACD) и Режим буст (частично те же данные)
            # → берём max, а не сумму, чтобы не считать одно явление дважды
            ...
            # Momentum — независим, суммируем; Breakout vs Regime — max
            combined_pos = mom_boost + max(bo_boost, reg_boost)
            # Hard cap на положительный сдвиг: не более +12%
            combined_pos = min(combined_pos, 12.0)

            # Штраф за неблагоприятный режим (применяется ПОСЛЕ всех бустов)
            penalty = 0.0
            if regime_name == "DOWNTREND":
                penalty = -12.0   # против тренда — финальный штраф, не перекрываем
            elif regime_name == "VOLATILE":
                penalty = -4.0

            total_boost = combined_pos + penalty  # penalty отрицательный
