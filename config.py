...
class Config:
    ...
    # GRINCH 20.07.2026: ATR(15m)=2.225% → 2.5×ATR=5.6% (TRAILING_STOP_PCT=9% > SL → floor активен)
    # ATR(1h)хар.≈4-5% → 2.5×ATR_1h=10-12.5% — динамич. SL шире базового → базовый (9%) берётся как пол
    ...
    # GRINCH 20.07.2026: ATR(15m)=2.225% → 2×ATR=4.45%; ATR(1h)хар.≈4-5% → 2×ATR=8-10%
    # Шорт-трейл должен пережить 1-2 свечи 1h → мин 9%; широкий = ловим полный откат.
    SHORT_TRAIL_PCT = float(os.getenv("SHORT_TRAIL_PCT", "9.0"))   # 2×ATR(1h)хар. — минимум против шума
